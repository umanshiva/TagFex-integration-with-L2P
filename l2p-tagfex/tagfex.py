...
class TagFex(ContinualLearner):
    def __init__(self, data_maganger, configs: dict, device, distributed=False, **kwargs) -> None:
        super().__init__(data_maganger, configs, device, distributed)

        self._init_network(self.configs.get('backbone_configs', dict()), self.configs.get('network_configs', dict()), **kwargss)
    ...
    def _init_network(self, backbone_configs, network_configs, **kwargs):
        ...
        self.network = TagFexNet(backbone_configs, network_configs, self.device, pretrained=True, **kwargs)
